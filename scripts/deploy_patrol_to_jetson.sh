...
AGENT="${GO2_JETSON_AGENT:-autonomy}"   # autonomy (video+roam+follow) | patrol (roam only)
VIDEO_PORT="${GO2_AUTONOMY_PORT:-8788}"
case "${AGENT}" in autonomy) UNIT="go2-autonomy" ;; patrol) UNIT="go2-patrol" ;; *) UNIT="go2-autonomy" ;; esac
...
SSHPASS="$GUN_JETSON_PASSWORD" sshpass -e ssh "${SSH_OPTS[@]}" -p "$PORT" "${JET_USER}@127.0.0.1" \
  "cd '${REMOTE_DIR}' && sudo -S -p '' env GO2_PATROL_GO_LIVE='${LIVE}' GO2_JETSON_AGENT='${AGENT}' bash deploy/jetson/install_patrol.sh" \
  <<< "${SUDO_PW}"

echo
echo "==> deployed agent: ${AGENT}  (${UNIT}.service)"
if [[ "$LIVE" == "1" ]]; then
  echo "==> LIVE: the Jetson is now autonomous. Stop with:"
  echo "    ssh -p ${PORT} ${JET_USER}@127.0.0.1 'sudo systemctl stop ${UNIT}'"
else
  echo "==> DRY RUN deployed (no motion). Re-run with --live to move."
fi
echo "    Follow logs: ssh -p ${PORT} ${JET_USER}@127.0.0.1 'journalctl -u ${UNIT} -f'"
if [[ "$AGENT" == "autonomy" ]]; then
  echo "    Watch video on your laptop: open a forward in another terminal, then http://127.0.0.1:${VIDEO_PORT}"
  echo "      sshpass -e ssh ${SSH_OPTS[*]} -L ${VIDEO_PORT}:${JET_HOST}:${VIDEO_PORT} ${DOG_USER}@${DOG_HOST}   # SSHPASS=dog password"
fi
